package IP_Rechner;
/* Klasse zur Berechnung der (bestmöglichen) Belegung der Subnetze und zur Vorbereitung der Ausgabe (VLSM)*/
...
public class SubnetzeAusgabeNetzwerk extends Netzwerk {
	...
	public SubnetzeAusgabeNetzwerk(int okt1, int okt2, int okt3, int okt4, int Prefix) {
		...
		// ArrayList für die Threads zur Instanziierung der Spalten
	...
	// Aktualisiert die Verfügbarkeit der Subnetze, nachdem ein Subnetz belegt wurde
	private void AktualisiereVerfuegbarkeitHinz(int IndexPrefixBasis, int IndexPosBasis) {
		 ...
		 * Ein Thread pro Prefix; Rückwärts: Ein Thread für alle Prefixe (Rückwärts muss
	...
	public void AuswahlFertig(int[] parNetzwerkeAusgewaehlt, int[] parNetzwerkeVerfuegbar) {
		...
		// ArrayList für die Threads zum Übertragen der ausgewählten Subnetze
		ArrayList<Thread> Threads = new ArrayList<Thread>();

		// Ausgewählte Subnetze (und ggf. Uplink-Netzwerk) von NetzwerkeVerfuegbar in
		// AusgewaehlteSubnetze übertragen (ein Thread pro Prefix)
		for (int i = 0; i < NetzwerkeAusgewaehlt.length; ++i) {
